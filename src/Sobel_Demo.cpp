...
int test_Sobel( int argc, char** argv )
{
  ...
  cv::CommandLineParser parser(argc, argv,
                               "{@input   |../test_imgs/lena.jpg|input image}"
  ...
  //![load]
  String str_src_img_path = parser.get<String>("@input");
  // As usual we load our source image (src)
  image = imread( str_src_img_path, IMREAD_COLOR ); // Load an image

  // Check if image is loaded fine
  if( image.empty() )
  {
    printf("Error opening image: %s\n", str_src_img_path.c_str());
    return 1;
  }
  //![load]
